...
DEFAULT_OUT_DIR = ROOT / "_build" / "bo3_rev_idg_weapon_only"
DEFAULT_WEAPON_GLB = ROOT / "_build" / "asset_port_pipeline" / "tmp" / "idg_view.glb"
DEFAULT_MANIFEST = TOOLS_DIR / "idg_weapon_only_rig_manifest.json"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build and validate a weapon-only BO3 IDG rig for BO2/T6.")
...
def main() -> None:
    ...
    output_glb = out_dir / "bo3_rev_idg_weapon_only.glb"
    metadata_out = out_dir / "weapon_only_rig_metadata.json"
    report_out = out_dir / "weapon_only_rig_report.json"
    preview_out = out_dir / "bo3_rev_idg_weapon_only_preview.blend"
    stdout_log = out_dir / "weapon_only_rig.stdout.log"
    stderr_log = out_dir / "weapon_only_rig.stderr.log"
    ...
    if not args.skip_build:
        ...
        if proc.returncode != 0:
            raise RuntimeError(f"Weapon-only rig Blender worker failed: exit={proc.returncode}")
    ...
    if args.preview:
        ...
        if preview_proc.returncode != 0:
            raise RuntimeError(f"Weapon-only rig preview failed: exit={preview_proc.returncode}")

    print(f"Weapon-only rig GLB: {output_glb}")
    print(f"Weapon-only rig report: {report_out}")
    if args.preview:
