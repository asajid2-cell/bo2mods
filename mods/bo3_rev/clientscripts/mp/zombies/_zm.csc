...
#/
    ...
    if ( level.scr_zm_ui_gametype_group == "zsurvival" && getdvar( #"ui_mapname" ) == "zm_transit" )
    {
        if ( level.scr_zm_map_start_location == "" )
        {
            level.scr_zm_map_start_location = "transit";
            setdvar( #"ui_zm_mapstartlocation", "transit" );
        }

        println( "[bo3_rev][csc] preserving client ui_gametype=" + level.scr_zm_ui_gametype + ";loc=" + level.scr_zm_map_start_location );
    }
...
#/
    ...
    self thread bo3_rev_local_anim_watch( localclientnum );
}
...
#/
    ...
    {
        if ( level.scr_zm_ui_gametype_group == "zclassic" && level.scr_zm_ui_gametype == "zclassic" && !level.createfx_enabled )
        {
...
bo3_rev_local_anim_watch( localclientnum )
{
    self endon( "entityshutdown" );
    ...
    if ( isdefined( self.bo3_rev_local_anim_watch_started ) )
        return;

    self.bo3_rev_local_anim_watch_started = 1;

    start_ms = gettime();
    end_ms = start_ms + 14000;
    last_model = "";
    last_flash = undefined;
    last_weapon = undefined;
    last_brass = undefined;
    last_clip = undefined;
    ...
    println(
        "[bo3_rev][csc_anim][t=" + gettime() + "] "
        + "stage=watch_begin"
        + ";lc=" + localclientnum
        + ";ent=" + bo3_rev_anim_debug_safe_str( self getentitynumber() )
        + ";model=" + bo3_rev_anim_debug_safe_str( self.model )
    );

    for ( sample_idx = 0; gettime() < end_ms; sample_idx++ )
    {
        flash_tag = self gettagorigin( "tag_flash" );
        weapon_tag = self gettagorigin( "tag_weapon" );
        brass_tag = self gettagorigin( "tag_brass" );
        clip_tag = self gettagorigin( "tag_clip" );
        flash_delta_sq = bo3_rev_anim_tag_delta_sq( last_flash, flash_tag );
        weapon_delta_sq = bo3_rev_anim_tag_delta_sq( last_weapon, weapon_tag );
        brass_delta_sq = bo3_rev_anim_tag_delta_sq( last_brass, brass_tag );
        clip_delta_sq = bo3_rev_anim_tag_delta_sq( last_clip, clip_tag );
        model_changed = self.model != last_model;
        should_log = sample_idx == 0 || sample_idx % 20 == 0 || model_changed || flash_delta_sq > 0 || weapon_delta_sq > 0 || brass_delta_sq > 0 || clip_delta_sq > 0;
        ...
        if ( should_log )
        {
            println(
                ...
                + ";tag_flash=" + bo3_rev_anim_debug_safe_str( flash_tag )
                + ";tag_weapon=" + bo3_rev_anim_debug_safe_str( weapon_tag )
                + ";tag_brass=" + bo3_rev_anim_debug_safe_str( brass_tag )
                + ";tag_clip=" + bo3_rev_anim_debug_safe_str( clip_tag )
        ...
        last_flash = flash_tag;
        last_weapon = weapon_tag;
        last_brass = brass_tag;
        last_clip = clip_tag;
        wait 0.05;
    }
