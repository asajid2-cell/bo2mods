...
#include clientscripts\mp\zombies\_bo3_rev_servant_fx_v3;
...
main()
{
    println( "[bo3_rev][csc] zm_transit main build=0321232501_4c7e1d" );
...
register_client_fields()
{
    println( "[bo3_rev][csc] zm_transit register_client_fields build=0321232501_4c7e1d" );
    if ( level.scr_zm_ui_gametype == "zclassic" )
    {
    ...
    // Must occur during the clientfield registration window (this function),
    // otherwise the server will disconnect with EXE_CLIENT_FIELD_MISMATCH.
    if ( !isdefined( level.bo3_rev_servant_clientfields_init ) )
    {
        level.bo3_rev_servant_clientfields_init = 1;
        clientscripts\mp\zombies\_bo3_rev_servant_fx_v3::init();
    }
}
...
start_zombie_stuff()
{
    ...
    // Keep the stock Transit burn overlay name/version alive so the server and
    // client register the same overlay clientfields, but neutralize the client
    // implementation while the custom FF lane is missing the stock
    // zombie_transporter_overlay dependency. Using style_none preserves the
    // overlay_slot/overlay_lerp contract without reaching the crashy burn path.
    clientscripts\mp\_visionset_mgr::vsmgr_register_overlay_info_style_none( "zm_transit_burn", 1, 15 );
}
