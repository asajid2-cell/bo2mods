...
void usage()
{
    fwprintf(stderr, L"Usage: fx_runtime_probe_injector.exe --dll <path> [--pid <pid> | --name <exe_name>]\n");
}
} // namespace
...
int wmain(int argc, wchar_t** argv)
{
    ...
    if (!pid)
    {
        ...
        if (!pid)
        {
            fwprintf(stderr, L"Could not find process named '%s'\n", proc_name);
            return 1;
        }
    }
    ...
    {
        fwprintf(stderr, L"Could not resolve DLL path '%s'\n", dll_path);
        return 1;
    }
    ...
    if (!proc)
    {
        fwprintf(stderr, L"OpenProcess failed for pid %lu (err=%lu)\n", pid, GetLastError());
        return 1;
    }
    ...
    if (!remote_buf)
    {
        fwprintf(stderr, L"VirtualAllocEx failed (err=%lu)\n", GetLastError());
        CloseHandle(proc);
        return 1;
    }
    ...
    {
        fwprintf(stderr, L"WriteProcessMemory failed (err=%lu)\n", GetLastError());
    ...
    if (!kernel32)
    {
        fwprintf(stderr, L"GetModuleHandleW(kernel32) failed\n");
        VirtualFreeEx(proc, remote_buf, 0, MEM_RELEASE);
        CloseHandle(proc);
        return 1;
    }
    ...
    if (!thread)
    {
        fwprintf(stderr, L"CreateRemoteThread failed (err=%lu)\n", GetLastError());
        VirtualFreeEx(proc, remote_buf, 0, MEM_RELEASE);
        CloseHandle(proc);
        return 1;
    }
    ...
    wprintf(L"Injected '%s' into pid %lu -> remote module 0x%08lX\n", full_dll, pid, remote_module);
